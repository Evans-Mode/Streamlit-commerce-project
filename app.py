...
[data-testid="stTab"] { font-size: 1rem; font-weight: 600; }
</style>
...
st.caption(
    "AWS Bedrock Nova Lite 2  · XGBoost via SageMaker · "
    "Policy RAG via Pinecone · Orders via Databricks"
)
...
with tab_chat:
    st.subheader("Ask the Refund Risk Agent", anchor=False)
    ...
    if st.session_state.last_result:
        ...
        st.info(
            "Enter a question in the box above and click **▶ Run Agent**.\n\n"
            "The agent will: look up the order → score refund risk via XGBoost/SageMaker "
            "→ retrieve policy from Pinecone → produce a grounded recommendation."
        )
