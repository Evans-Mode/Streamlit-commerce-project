...
# Configuration
# Model is configured directly in get_model() — no env var needed for the name.
...
def get_model():
    ...
    if _llm is None:
        _llm = ChatOpenAI(
            ...
            max_tokens=512,  # default ceiling; nodes that need fewer use .bind()
        )
    return _llm

def invoke_with_retry(model, messages, max_retries: int = 3, **kwargs):
    # ChatOpenAI / Kimi: max_tokens and temperature must live in the constructor,
    # not as per-call kwargs. Strip them here so callers don't need to know.
...
than 1000 characters and converted to strings, never raw JSON or code blocks, to ensure clean display in the UI. 
...
def supervisor_node(state: AgentState) -> dict:
    ...
    # .content can be a list of blocks on some OpenAI-compatible APIs
    raw = routing_response.content
    ...
    # Primary: first whitespace-delimited token
    first_token = raw.split()[0] if raw.split() else ""
    route = first_token if first_token in valid else None

    # Fallback: scan the whole response for a known keyword
    if route is None:
        for word in valid:
            if word in raw:
                route = word
    ...
    # Final fallback: default to respond
    if route is None:
        route = "respond"

    # Auto-chain: if lookup was chosen but features already exist, skip to score
...
def respond_node(state: AgentState) -> dict:
    """Final synthesis for the operator, strictly enforcing the 1000-character limit."""
    model = get_model()
        ...
        [
            {
                ...
                "content": (
                    ...
                    "CRITICAL RULES: "
                    "1. Keep all answers to business language, not customer-facing. "
                    "2. Your final output MUST be less than 1000 characters. "
                    ...
                    "5. If requested information is missing in policy, say 'Answer not in scope' instead of making it up."
                )
            },
